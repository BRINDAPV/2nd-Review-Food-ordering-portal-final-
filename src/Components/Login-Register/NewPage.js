...
const Register = ({ onSwitchToLogin }) => {
  ...
  const changeMessage2 = async () => {
    ...
    if (!validateConfirm2(Rtpassword, RtconfirmPassword)) {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8081/users', { email: Rtemail, password: Rtpassword });

      if (response.data.success) {
        setRegisterationSuccess(true);
        history2.push('/RegisteredUser');
      } else {
